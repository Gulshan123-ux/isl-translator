...
def extract_both_hands_keypoints(frame_bgr, hand_landmarker) -> np.ndarray:
...
def load_dataset_both_hands(data_dir: Path, max_frames: int = 30) -> Tuple[np.ndarray, np.ndarray, List[str]]:
    """Scan class subfolders for images (.jpg/.png) or videos (.mp4/.mov/.mkv) and extract (N, T, 126) keypoints.

    For image folders, consecutive images are grouped into sequences of max_frames.
    For video files, frames are extracted directly.
    """
    ...
    class_dirs = sorted([d for d in os.listdir(data_dir) if (data_dir / d).is_dir()])
    for ci, label in enumerate(class_dirs):
        class_dir = data_dir / label
        all_files = sorted(os.listdir(class_dir))

        # --- Detect whether this class folder has images or videos ---
        images = [f for f in all_files if f.lower().endswith(IMAGE_EXTS)]
        videos = [f for f in all_files if f.lower().endswith(VIDEO_EXTS)]

        # ---- Handle IMAGE folders (group consecutive images into sequences) ----
        if images:
            ...
            # Group images into non-overlapping sequences of max_frames
            for start in range(0, len(images), max_frames):
                chunk = images[start:start + max_frames]
                frames = []
                for fname in chunk:
                    img = cv2.imread(str(class_dir / fname))
                    if img is None:
                        continue
                    kp = extract_both_hands_keypoints(img, hand_landmarker)
                    frames.append(kp)
                if len(frames) == 0:
                    continue
                ...
                if len(frames) < max_frames:
                    ...
                    frames.extend(pad)
                arr = np.asarray(frames[:max_frames], dtype=np.float32)
        ...
        for fname in videos:
            ...
            while cap.isOpened() and len(frames) < max_frames:
                ...
                if not ret:
                    break
                kp = extract_both_hands_keypoints(frame, hand_landmarker)
                frames.append(kp)
            cap.release()
            if len(frames) == 0:
                continue
        ...
        print(f"  [{ci+1}/{len(class_dirs)}] class '{label}': {len([v for v in y if v == label])} sequences")
...
def main():
    ...
    parser.add_argument('--data_dir', type=str, default='Indian', help='Dataset root with class folders containing images or videos.')
    ...
    X, y, label_names = load_dataset_both_hands(data_dir, max_frames=args.max_frames)
    if len(X) == 0:
